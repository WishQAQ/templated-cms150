...
$config = array(
    ...
    'use_boder'   => TRUE,
    'font_file'   => dirname(__FILE__).'/data/fonts/ariblk.ttf',
...
function echo_validate_image( $config = array() )
{
    ...
    $img_height  = isset($config['img_height']) ? $config['img_height'] : 25;
    $img_width   = isset($config['img_width']) ? $config['img_width'] : 60;
    $font_file   = isset($config['font_file']) ? $config['font_file'] : PATH_DATA.'/data/font/ariblk.ttf';
    $use_boder   = isset($config['use_boder']) ? $config['use_boder'] : TRUE;
    $filter_type = isset($config['filter_type']) ? $config['filter_type'] : 5;
    
    //创建图片，并设置背景色
    $im = @imagecreate($img_width, $img_height);
    imagecolorallocate($im, 41,34,25);
    
    //文字随机颜色
    $fontColor[]  = imagecolorallocate($im, 0xff, 0xff, 0xff);
    $fontColor[]  = imagecolorallocate($im, 0xff, 0xff, 0xff);
    $fontColor[]  = imagecolorallocate($im, 0xff, 0xff, 0xff);
    $fontColor[]  = imagecolorallocate($im, 0xff, 0xff, 0xff);
    $fontColor[]  = imagecolorallocate($im, 0xff, 0xff, 0xff);
